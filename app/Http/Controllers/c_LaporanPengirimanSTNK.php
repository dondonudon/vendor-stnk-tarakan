...
class c_LaporanPengirimanSTNK extends Controller
{
    public function index() {
        return view('dashboard.report.pengiriman-stnk.index');
    }

    public function data($start,$end,$dealer) {
        $dataset = DB::table('po_stnk_dealer')
            ->select(
            ...
            ->join('po_trns','po_stnk_dealer.id_trn','=','po_trns.id')
            ->join('po_mst','po_stnk_dealer.no_po','=','po_mst.no_po')
...
//            $sheet->mergeCells('A'.$i.':D'.$i);
            ...
            $response->headers->set('Content-Disposition', 'attachment;filename="laporan-penerimaan-stnk-oleh-dealer.xlsx"');
...
//            return response()->json($ex);
    ...
    public function exportPDF($start,$end,$dealer) {
        ...
        try {
            $pdf = PDF::loadView('dashboard.report.pengiriman-stnk.pdf',$trn)->setPaper('a4','portrait');
            return $pdf->stream('laporan-pengiriman-stnk.pdf');
        } catch (\Exception $ex) {
            dd($ex);
        }
    }

...
class c_LaporanPengirimanPlat extends Controller
{
    public function index() {
        return view('dashboard.report.pengiriman-plat.index');
    }

    public function data($start,$end,$dealer) {
        $dataset = DB::table('po_plat_dealer')
            ->select(
            ...
            ->join('po_trns','po_plat_dealer.id_trn','=','po_trns.id')
            ->join('po_mst','po_plat_dealer.no_po','=','po_mst.no_po')
    ...
    public function exportExcel($start,$end,$dealer) {
        ...
        try {
            ...
            $sheet->setCellValue('A2','SERAH TERIMA PLAT Nomor '.join(', ',$dtDealer));
            ...
            $response->headers->set('Content-Disposition', 'attachment;filename="laporan-penerimaan-plat-nomor-oleh-dealer.xlsx"');
...
//            return response()->json($ex);
    ...
    public function exportPDF($start,$end,$dealer) {
        ...
        try {
            $pdf = PDF::loadView('dashboard.report.pengiriman-plat.pdf',$trn)->setPaper('a4','portrait');
            return $pdf->stream('report-pengiriman-plat.pdf');
        } catch (\Exception $ex) {
            dd($ex);
        }
    }

...
class c_TransaksiPlatSamsat extends Controller
{
    public function index() {
        return view('dashboard.transaction.plat-dari-samsat.list');
    }
    ...
    public function validasi($nopo) {
        ...
        if ($mst->exists()) {
            $data['mst'] = $mst->first();
            return view('dashboard.transaction.plat-dari-samsat.validasi')->with('data',$data);
        } else {
            abort(404);
        }
    }
        ...
        try {
            ...
            foreach ($kendaraan as $k) {
                DB::table('po_trns')
                    ->where('id','=',$k['id'])
                    ->update([
                        'status_plat_samsat' => $k['status'],
                    ]);

                DB::table('po_plat_samsat')
                    ->where('id_trn','=',$k['id'])
                    ->update([
                        'status' => $k['status'],
                        'tgl_validasi' => $today,
